...
from .UNetBlocks import CirUnetBlock, Config as Config_
from torch_utils.torchutils import ModuleManager
import functools
...
class NetD(nn.Sequential):
    ...
    def __init__(self, in_ch=1, hidden_ch=64, n_layers=3, norm_layer=nn.BatchNorm2d):
...
class pix2pixGAN(nn.ModuleList):
    """refer to:
    paper:
    ...
    def __init__(self, in_ch=1, net_g_config=Config.net_g_config, net_d_config=Config.net_d_config,
        ...
        self.net_d = net_d if net_d is not None else NetD(**net_d_config)
    ...
    def loss_d(self, real, fake):
        ...
        pred_fake = self.net_d(fake.detach())  # G won't be updated
